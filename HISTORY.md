...
## [2026-04-21] 보안 감사 및 릴리즈 최적화 (v1.0.1)

### 작업 내용
- **보안 감사 및 개선:**
    - `AndroidManifest.xml`: 앱 데이터 백업 비활성화(`allowBackup=false`)로 보안 강화.
    - `build.gradle.kts`: **R8 난독화 및 리소스 최적화** 활성화 (코드 보호 및 용량 감소).
    - 패키지명 정규화: `com.example`에서 `com.jeiel.open_weather`로 고유 ID 변경.
- 모든 기능 정상 동작 확인 및 최종 안정화 버전 v1.0.1 배포.

### 현재 상태
- **보안 최적화가 완료된 v1.0.1 출시.**
- 플레이 스토어 출시를 위한 모든 기술적 준비 완료.
