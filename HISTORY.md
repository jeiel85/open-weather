...
## [2026-04-21] GitHub Pages 빌드 오류 수정 및 워크플로우 개선

### 작업 내용
- **GitHub Pages 워크플로우(`pages.yml`) 개선:**
    - `workflow_dispatch`를 추가하여 GitHub Actions 탭에서 수동으로 배포를 재시도할 수 있도록 개선.
    - 빌드 단계에 `ls -R website` 명령어를 추가하여 파일 경로 및 존재 여부를 디버깅할 수 있도록 보완.
- **배포 가이드(`DEPLOYMENT.md`) 업데이트:**
    - GitHub Pages 설정 시 "Source"를 "GitHub Actions"로 설정해야 함을 명시.

### 현재 상태
- APK/AAB 릴리즈는 정상적으로 수행됨.
- 페이지 빌드 오류는 워크플로우 수정 및 저장소 설정 확인을 통해 해결 중.
