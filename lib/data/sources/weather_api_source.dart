...
class WeatherApiSource {
  ...
  Future<WeatherModel> fetchWeather(double lat, double lon, String locationName) async {
    ...
    if (response.statusCode == 200) {
      ...
      return await _fetchAirQuality(lat, lon, weather);
    } else {
  ...
  Future<WeatherModel> _fetchAirQuality(double lat, double lon, WeatherModel weather) async {
    try {
      final url = Uri.parse(
      ...
      debugPrint('AQI 정보를 불러오는 데 실패했습니다: $e');
    }
